...
#+begin_src plantuml :file images/afspm_hl_diagram.png
  ...
  package "afspm abstraction" <<groupinternal>> {
    ...
    component "microscope\nmediator\n<$ma_tune>" <<internal>> <<required>> as mediator
...
#+begin_src plantuml :file images/afspm_hl_example.png
  ...
  package "afspm abstraction" <<groupinternal>> as afspm {
    ...
    mediator -l-> exp: scope state
    mediator --> roi: scans
    roi -l-> exp: roi points
    exp -r-> mediator: scan\nparameters

    mediator -r-> tip_detector: scans
    tip_detector .l.> mediator: log tip\nproblem
...
#+begin_src plantuml :file images/afspm_hl_example2.png
  ...
  package "generic abstraction" <<groupinternal>> as afspm {
    ...
    mediator -l-> feedback_analyzer: scans
    feedback_analyzer .r.> mediator: log\nproblem

    exp -[hidden]down- feedback_analyzer

    mediator -d-> feedback_optimizer: problems
    feedback_optimizer .u.> mediator: feedback\nweights


    mediator -r-> tip_detector: scans
    tip_detector .l.> mediator: log\nproblem

    mediator -d-> tip_corrector: problems
    tip_corrector .u.> mediator: corrective\nactions
  }
...
//PUSH-PULL//: pipeline distribution model"
  ...
  :end:
* afspm without microscope mediator

#+header: :post attr_fit_width(data=*this*, org_percent=75, latex_percent=90)
#+begin_src plantuml :file images/afspm_without_mediator.png
  !include style.puml
  !include microscopy.puml
  !include <material/network>
  !include <material/script>
  ...
  [[file:images/afspm_without_mediator.png]]
  :end:
...
#+begin_src plantuml :file images/afspm_control.png
  ...
  package "afspm abstraction" <<groupinternal>> {
    ...
    component "microscope\nmediator\n<$ma_tune>" <<internal>> <<required>> as mediator {
      component XPUB <<socket>> as mediator_pub
      component SUB <<socket>> as mediator_sub
      component "//cache//" as mediator_cache
      mediator_sub .. mediator_cache
      mediator_cache .. mediator_pub

      component ROU <<socket>> as mediator_rou
      component REQ <<socket>> as mediator_req
      component "//control//" as mediator_ctrl
      mediator_req .. mediator_ctrl
      mediator_ctrl .. mediator_rou
    }

    dc_pub -d-> mediator_sub: data, \nstatus
    dc_rep <-d- mediator_req: control\nrequests
    ...
    mediator_rou <-- exp_req: "scan parameters,\nstart / stop scan requests"
    mediator_pub --> exp_sub: scope state
    ...
    mediator_pub --> viz_sub: scope state
  }
