...
- *Microscope Mediator*: mediates control between components and translator and caches data.
...
- Messages may be *cached* by the Mediator, to send to new components.
#+beamer:\pause
*** Control Path
- Components send *requests* to the SPM and receive *responses*.
- The Mediator *routes* control to the SPM (one component at a time).
